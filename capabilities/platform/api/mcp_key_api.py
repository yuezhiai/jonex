...
async def create_mcp_key(
    ...
    _admin: dict = Depends(require_permission("mcp:key:manage")),
):
...
async def list_mcp_keys(
    request: Request,
    db: AsyncSession = Depends(get_db),
    _admin: dict = Depends(require_permission("mcp:key:view")),
):
...
async def revoke_mcp_key(
    ...
    authorization_header = request.headers.get("Authorization")
    svc = McpKeyService(db)
    result = await svc.revoke(tenant_id, key_id, authorization_header)
    return success_response(data=result, message="MCP Key 已撤销")


@router.post("/mcp-keys/{key_id}/toggle")
...
@router.post("/mcp-keys/{key_id}/recreate")
async def recreate_mcp_key(
    key_id: str = Path(..., description="MCP Key ID (32 字符 hex)"),
    request: Request = None,
    body: McpKeyRecreateRequest | None = Body(
        None, description="可选：覆盖新 Key 的 expires_at / name"
    ),
    db: AsyncSession = Depends(get_db),
    _admin: dict = Depends(require_permission("mcp:key:manage")),
):
    """重新创建 MCP Key（仅过期态可用）：继承授权 + 生成新明文 Key，旧 Key 保留历史。
    ...
    result = await svc.recreate(
        tenant_id, key_id, authorization_header, expires_at=expires_at, name=name
    )
    return success_response(
        data=McpKeyCreateResponse(**result).dict(),
